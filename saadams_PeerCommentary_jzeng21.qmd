...
```{r}
1 - ppois(7, 15) # P(calls >= 8) = 1 - P(calls < 8) #Jason- I want to first say I love your notes and picture section above I think I am going to take inspiration on your note section and also make a note section of my own in my next homework! I think it really helps with going back to recall what each function actually does! I did put 8 instead of 7 here.
```
...
```{r}
dpois(0, 15) # P(calls = 0) # Similar but I did use ppois here instead of dpois but I think both works? Maybe you are right then me
```
...
```{r}
barplot(dpois(x=0:30, lambda=15), ylim=c(0,0.15), space=0, names.arg=0:30, col = "purple", ylab="P(X=x)", xlab="# of Calls") #Nice simple bar plot!
```
...
summary(simulation)  #Nice overall and nice set.seed usage
```
